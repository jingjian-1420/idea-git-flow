...
public class FinishReleaseAction extends GitflowAction {
  ...
  public void actionPerformed(AnActionEvent e) {
    ...
    if (currentBranchName.isEmpty() == false) {
      ...
      releaseName = customReleaseName != null ? customReleaseName
          : GitflowConfigUtil.getReleaseNameFromBranch(myProject, myRepo, currentBranchName);
      ...
      String customTagMessage = GitflowConfigurable
          .getOptionTextString(myProject, "RELEASE_customTagCommitMessage");
      ...
      if (GitflowConfigurable.isOptionActive(myProject, "RELEASE_dontTag")) {
        ...
        tagMessageDraft = Messages
            .showInputDialog(myProject, "Enter the tag message:", "Finish Release",
        ...
        new Task.Backgroundable(myProject, "Finishing release " + releaseName, false) {
          ...
          public void run(@NotNull ProgressIndicator indicator) {
            GitCommandResult result = myGitflow
                .finishRelease(myRepo, releaseName, tagMessage, errorLineHandler);

            if (result.success()) {
              String finishedReleaseMessage = String
                  .format("The release branch '%s%s' was merged into '%s' and '%s'", releasePrefix,
          ...
          public void onSuccess() {
            ...
            if (errorLineHandler.hasMergeError) {
              if (handleMerge()) {
                FinishReleaseAction completeFinisReleaseAction = new FinishReleaseAction(
                    releaseName, tagMessage);
