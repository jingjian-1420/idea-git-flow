...
import cn.rieon.idea.git.flow.ui.GitflowOptionsForm;
  ...
  public void apply() throws ConfigurationException {
    ...
    for (GitflowOptionsFactory.TYPE type : GitflowOptionsFactory.TYPE.values()) {
      for (Map<String, String> optionMap : gitflowOptions.get(type)) {
        ...
        // set isActive value
        propertiesComponent
            .setValue(optionId + "_active", gitflowOptionsForm.isOptionActive(optionId));

        // set text value, if relevant
        if (optionMap.get("inputText") != null) {
          propertiesComponent
              .setValue(optionId + "_text", gitflowOptionsForm.getOptionText(optionId));
        }
