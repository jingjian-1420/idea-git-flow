...
public class PublishFeatureAction extends GitflowAction {
  ...
  public void actionPerformed(AnActionEvent anActionEvent) {
    ...
    final String featureName = GitflowConfigUtil
        .getFeatureNameFromBranch(myProject, myRepo, currentBranchName);

    new Task.Backgroundable(myProject, "Publishing feature " + featureName, false) {
      @Override
      public void run(@NotNull ProgressIndicator indicator) {
        GitCommandResult result = myGitflow
            .publishFeature(myRepo, featureName, new GitflowErrorsListener(myProject));

        if (result.success()) {
          String publishedFeatureMessage = String
              .format("A new remote branch '%s%s' was created", featurePrefix, featureName);
