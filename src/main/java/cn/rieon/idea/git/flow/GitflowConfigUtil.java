...
public class GitflowConfigUtil {
  ...
  public static String getFeatureNameFromBranch(Project project, GitRepository repo,
      String branchName) {
    String featurePrefix = GitflowConfigUtil.getFeaturePrefix(project, repo);
    return branchName
        .substring(branchName.indexOf(featurePrefix) + featurePrefix.length(), branchName.length());
  }

  public static String getReleaseNameFromBranch(Project project, GitRepository repo,
      String branchName) {
    String releasePrefix = GitflowConfigUtil.getReleasePrefix(project, repo);
    return branchName
        .substring(branchName.indexOf(releasePrefix) + releasePrefix.length(), branchName.length());
  }

  public static String getHotfixNameFromBranch(Project project, GitRepository repo,
      String branchName) {
    String hotfixPrefix = GitflowConfigUtil.getHotfixPrefix(project, repo);
    return branchName
        .substring(branchName.indexOf(hotfixPrefix) + hotfixPrefix.length(), branchName.length());
  }
