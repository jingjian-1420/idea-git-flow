...
import cn.rieon.idea.git.flow.GitflowInitOptions;
import cn.rieon.idea.git.flow.ui.GitflowInitOptionsDialog;
...
public class InitRepoAction extends GitflowAction {
  ...
  public void actionPerformed(AnActionEvent e) {
    ...
    GitflowInitOptionsDialog optionsDialog = new GitflowInitOptionsDialog(myProject,
        branchUtil.getLocalBranchNames());
    optionsDialog.show();
      ...
      new Task.Backgroundable(myProject, "Initializing repo", false) {
        ...
        public void run(@NotNull ProgressIndicator indicator) {
          GitCommandResult result = myGitflow
              .initRepo(myRepo, initOptions, errorLineHandler, localLineHandler);

          if (result.success()) {
            String publishedFeatureMessage = String
                .format("Initialized gitflow in repo " + myRepo.getRoot().getPresentableName());
          ...
          //update the widget
          myProject.getMessageBus().syncPublisher(GitRepository.GIT_REPO_CHANGE)
              .repositoryChanged(myRepo);
          myRepo.update();
        }
      }.queue();
    }
