...
import cn.rieon.idea.git.flow.ui.GitflowStartFeatureDialog;
...
public class StartFeatureAction extends GitflowAction {
  ...
  private GitCommandResult createFeatureBranch(String baseBranchName, String featureName) {
    ...
    GitCommandResult result = myGitflow
        .startFeature(myRepo, featureName, baseBranchName, errorListener);

    if (result.success()) {
      String startedFeatureMessage = String
          .format("A new branch '%s%s' was created, based on '%s'", featurePrefix, featureName,
              baseBranchName);
