    ...
    if (remoteBranches.size() > 0) {
      ...
      if (branchChoose.isOK()) {
        ...
        final String featureName = GitflowConfigUtil
            .getFeatureNameFromBranch(myProject, myRepo, branchName);
        ...
        new Task.Backgroundable(myProject, "Tracking feature " + featureName, false) {
          ...
          public void run(@NotNull ProgressIndicator indicator) {
            GitCommandResult result = myGitflow
                .trackFeature(myRepo, featureName, remote, errorLineHandler);

            if (result.success()) {
              String trackedFeatureMessage = String
                  .format("A new branch '%s%s' was created", featurePrefix, featureName);
